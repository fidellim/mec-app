...
- Admins and Super Admins can set an employee-specific current-year base annual leave override in **Manage Users** for employees who need a different base `L100 - Annual Leave` entitlement from the regional or service-based rule. A blank override uses the normal default. Use **Annual Carry-Overs** for unused prior-year leave days.
- Admins and Super Admins manage annual leave carry-overs in **Annual Carry-Overs**. Manual opening balances and CSV imports are approved immediately; generated year-end suggestions remain pending until Admin or Super Admin approves them. **Unused leave from year** records where the days came from; **Apply to year** is the year that receives the approved days.
- Approved annual leave carry-over adds to the target year's `L100 - Annual Leave` allowance. Pending, rejected, and voided carry-over records do not affect balances. Admins and Super Admins void incorrect approved carry-overs with a required reason instead of deleting them.
- Generated carry-over suggestions calculate remaining `L100` balance from the selected year and suggest it for the following year. For example, generating from `2026` creates pending suggestions for `2027`. Admin or Super Admin must review and approve the rows before they affect balances.
- Leave allowances refresh automatically by year. Unused allowance does not carry over unless Admin or Super Admin records or approves an annual leave carry-over for the next year.
