...
- Eligible entitlement balances are shown on dashboards, leave forms, and user profiles. Maternity leave is available only when the employee gender is set to Female. Parental leave is available only when marital status is set to Married. A separate paternal leave code has not been added; the marital-status rule applies to the existing `L170 - Parental Leave` code.
