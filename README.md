...
- `L100 - Annual Leave`, `L110 - Sick Leave`, `L160 - Maternity Leave`, `L170 - Parental Leave`, Philippines `L180 - Bereavement / Compassionate Leave`, and Philippines-only `L190 - Service Incentive Leave` have entitlement limits. UAE `L180` uses relationship-based per-request limits for spouse and immediate-family bereavement. Super Admin sets UAE and Philippines defaults in **Leave Settings**, including UAE sick and maternity maximum claimable calendar-day limits, and can set per-user annual overrides in **Manage Users**.
...
- UAE bereavement leave requires the employee to select the relationship to the deceased. The configured spouse and immediate-family limits are checked against each submitted request rather than a yearly balance.
